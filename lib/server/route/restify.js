...
var VERBS = ['list', 'get', 'post', 'put', 'delete'],
    ROLES = ['any', 'guest', 'user', 'manager', 'superuser', 'admin'];
...
Restify.prototype.manager = function (path, controller, verbs) {
  var self = this;
  self._attachRoutes(path, controller, verbs, function (req, res, next) {
    if (!req.isAuthenticated()) {
      return self._sendError(new Error('Must be logged in.'), null, res, 401);
    }
    if (!req.user || !_.intersection(req.user.roles, ['admin', 'superuser', 'manager']).length) {
      return self._sendError(new Error('Must be manager.'), null, res, 403);
    }
    next();
  });
};
Restify.prototype.superuser = function (path, controller, verbs) {
  var self = this;
  self._attachRoutes(path, controller, verbs, function (req, res, next) {
    if (!req.isAuthenticated()) {
      return self._sendError(new Error('Must be logged in.'), null, res, 401);
    }
    if (!req.user || !_.intersection(req.user.roles, ['admin', 'superuser']).length) {
      return self._sendError(new Error('Must be superuser.'), null, res, 403);
    }
    next();
  });
};
Restify.prototype.admin = function (path, controller, verbs) {
  ...
  self._attachRoutes(path, controller, verbs, function (req, res, next) {
    ...
    if (!req.user || !_.intersection(req.user.roles, ['admin']).length) {
...
Restify.prototype.model = function (path, modelName, overrideController) {
  ...
  ['any', 'guest', 'user', 'manager', 'superuser', 'admin'].forEach(function (role) {
